...
def func(x, b):
    return 1/(np.sqrt(1-b*x*x))

# Импорт данных
data = import_data('data.csv')
N, time = get_columns(data)
...
params, cov = approximation(curve, N, time)

# Подписи к графику
plt.ylabel(r"Time, ms")
plt.xlabel(r"Number of threads")
plt.plot(N, time, '.', color = 'black', label = '', alpha = 0.25)

## Построение графика
ax = plt.gca()

## Сохранение графика
save_figure('graph1.png')
